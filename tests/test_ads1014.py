...
use simulated device:
  pdm run pytest
use hardware device:
  TEST_ADS1014_CHIP=y pdm run pytest
"""
...
if os.environ.get('TEST_ADS1014_CHIP', 'n') == 'y':
    HAS_HARDWARE = True
else:
    HAS_HARDWARE = False


class TestAds1014(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.ads1014 = sut.Ads1014(i2c_bus=self.i2c_bus)

    def tearDown(self):
        # restore original state after each run
        # (hardware is not stateless)
        self.ads1014.reset_device_registers()
    ...
    def test_ssc_measurement_default(self):
        ...
        # -----------------------------------------------------------------
        computed = self.ads1014.get_ssc_measurement(unit=UNIT.STEPS)
        expected = 0x1234
        # -----------------------------------------------------------------
    ...
    def test_ssc_measurement_in_microvolts(self):
        ...
        # -----------------------------------------------------------------
        computed = self.ads1014.get_ssc_measurement()
    ...
    def test_ssc_measurement_with_config(self):
        ...
        self.ads1014.get_ssc_measurement(config=config)
        # -----------------------------------------------------------------
        with BurstHandler(i2c_bus=self.i2c_bus, i2c_adr=self.i2c_adr) as bh:
    ...
    def test_ssc_measurement_continued(self):
        config = sut.Ads1014Config()
        self.ads1014.get_ssc_measurement(config=config)
        self.ads1014.get_ssc_measurement()  # use same config as previous read
        # -----------------------------------------------------------------
        with BurstHandler(i2c_bus=self.i2c_bus, i2c_adr=self.i2c_adr) as bh:
            computed = bh.read_register(0x01, byte_count=2)
        expected = [0x0583, 0x8583]
    ...
    def test_ssc_measurement_conflict1(self):
        config_ccm = sut.Ads1014Config(dom=sut.DOM.CCM)
        # -----------------------------------------------------------------
        # -----------------------------------------------------------------
        # can't use singleshot with continuous conversion mode
        self.assertRaises(RuntimeError, self.ads1014.get_ssc_measurement, config=config_ccm)
    ...
    def test_ssc_measurement_conflict2(self):
        ...
        self.assertRaises(RuntimeError, self.ads1014.get_ssc_measurement)

    # ---------------------------------------------------------------------

    def test_configure_verify_return_value(self):
        config = sut.Ads1014Config(dom=sut.DOM.CCM)
        # -----------------------------------------------------------------
        # -----------------------------------------------------------------
        self.assertTrue(self.ads1014.configure(config=config))

    def test_configure(self):
        config = sut.Ads1014Config(dom=sut.DOM.CCM)
        config.set_atlo(0xA000, unit=UNIT.STEPS)
        config.set_athi(0x5FFF, unit=UNIT.STEPS)
        self.ads1014.configure(config=config)
        # -----------------------------------------------------------------
        # -----------------------------------------------------------------
        with BurstHandler(i2c_bus=self.i2c_bus, i2c_adr=self.i2c_adr) as bh:
    ...
    def test_ccm_measurement(self):
        self.i2c_bus._state[0x48][0x00] = 0x1234
        self.ads1014.configure(config=sut.Ads1014Config(dom=sut.DOM.CCM))
        # -----------------------------------------------------------------
        computed = self.ads1014.get_ccm_measurement(unit=UNIT.STEPS)
        expected = 0x1234
        # -----------------------------------------------------------------
        self.assertEqual(expected, computed)
    ...
    def test_ccm_measurement_pga(self):
        self.i2c_bus._state[0x48][0x00] = 0x1234
        self.ads1014.configure(config=sut.Ads1014Config(dom=sut.DOM.CCM, pga=sut.PGA.MODE1))
        # -----------------------------------------------------------------
        computed = self.ads1014.get_ccm_measurement(unit=UNIT.MICRO)
        expected = 582518
        # -----------------------------------------------------------------
        self.assertEqual(expected, computed)
    ...
    def test_ccm_measurement_conflict(self):
        self.i2c_bus._state[0x48][0x00] = 0x1234
        self.ads1014.configure(config=sut.Ads1014Config(dom=sut.DOM.SSM))
        # -----------------------------------------------------------------
        # -----------------------------------------------------------------
        self.assertRaises(RuntimeError, self.ads1014.get_ccm_measurement)
